...
### Added
- `PopScreenContent` `dragMode` prop (`'handle' | 'header' | 'body'`) and
  `setDragMode()` API — choose whether the overlay is dragged from the top
  drag-handle strip, the header only, or the whole body.
